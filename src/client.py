...
def date_format_change(d: datetime.date) -> str: # Converts YYYY-MM-DD to M/D/YYYY for the dtdate parameter in the URL
...
def validate_date(date_str: str) -> datetime.date: # Must be within next week (7 days)
    try:
...
def fetch_menu_html(location_num: int, date: str) -> str: # No fallback option
    ...
    url = requests.get(
        BASE_URL, 
        params={'locationNum': location_num, 'dtdate': dtdate},
        timeout = 20,
